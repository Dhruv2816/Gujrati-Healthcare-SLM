...
from src.config import EMERGENCY_KEYWORDS, EMERGENCY_RESPONSE
...
class GraphRAGRetriever:
    ...
    def retrieve(self, query: str, top_k: int = 5) -> dict:
        ...
        primary_entity = (
            (entities.diseases or entities.symptoms or entities.drugs or [None])[0]
        )
...
def _build_context(vector_results: list[dict], kg_results: dict, entities) -> str:
    ...
    if vector_results:
        ...
        for i, r in enumerate(vector_results[:5], 1):
            parts.append(f"[{i}] ({r['source']}, relevance={r['score']:.2f})\n{r['text'][:400]}")

    # KG structured facts
    if kg_results:
        parts.append("\n## Medical Knowledge Graph Context")
        if kg_results.get("possible_diseases"):
            parts.append(f"Related Diseases: {', '.join(kg_results['possible_diseases'][:8])}")
        if kg_results.get("suggested_drugs"):
            parts.append(f"Common Drugs: {', '.join(kg_results['suggested_drugs'][:8])}")
        if kg_results.get("symptoms"):
            parts.append(f"Associated Symptoms: {', '.join(kg_results['symptoms'][:8])}")
        if kg_results.get("suggested_treatments"):
            parts.append(f"Treatments: {', '.join(kg_results['suggested_treatments'][:8])}")
